...
const DisneyPlusScreen = ({ navigation }) => {
  ...
  const { isSearchingPopular, error404Popular, searchPopular, disneyPlusPopular } =
    appContext;
  ...
  const onRefresh = useCallback(async () => {
    setRefreshing(true);
    setisSearching(true);
    let res = await searchPopular("disney-plus");
  ...
  if (!disneyPlusPopular.length || refreshing) {
    return (
