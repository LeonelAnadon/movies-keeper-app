...
const AmazonPrimeScreen = ({ navigation }) => {
  const appContext = useContext(MoviesContext);
  const { isSearchingPopular, error404Popular, searchPopular, amazonPrimePopular } =
    appContext;
  ...
  const onRefresh = useCallback(async () => {
    setRefreshing(true);
    setisSearching(true);
    let res = await searchPopular("amazon-prime-video");
  ...
  if (!amazonPrimePopular.length || refreshing) {
    return (
  ...
  return (
    <View style={styles.container}>
            {error404Popular === 'amazon-prime-video' ? (
        <View
          style={{
      ...
      <View>
        <FlatList
          ...
          ListFooterComponent={() => {
            if (!amazonPrimePopular?.length > 0)
